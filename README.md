...
$$
\begin{align}
  \text{Spot}_{t} = c + b_{1}\text{wind}_{t} + b_{2}\text{pv}_{t} + \sum_{k=1}^{23} b_{k+2}dh_{kt} + \sum_{l=1}^{6} b_{l+25}dd_{lt} + \sum_{m=1}^{11} b_{m+31}dm_{mt} + \sum_{n=1}^{3} b_{m+42}dy_{nt} + u_{t}
\end{align}
$$

Where:
- $$dh_{kt}, dd_{lt}, dm_{mt}, dy_{nt} = \text{dummy variables for hour, day, month, and year}$$
- $$\text{Spot}_{t} = \text{electricity spot price at time t}$$
- $$\text{wind}_{t} = \text{wind and solar feed in}$$
- $\text{pv}_{t} = \text{wind and solar feed in}$
